<x-app-layout>
    ...
    <main
		...
		<div
			...
			duration-500 ease-in-out">
			<h2 class="text-4xl font-medium capitalize">Cashouts ({{ count($c)}})</h2>
			<div class="mt-1 mb-4 flex items-center justify-between">
				<span class="text-sm">

					<strong class="text-blue-300"> {{count($c)}}</strong> |
                    <strong class="text-green-700"> {{count($c1)}}</strong>
				</span>
			...
			<div class="flex flex-col mt-2">
                @foreach($c1 as $cash_out)
                    <div class="flex flex-row mt-2">
                        <div
                            class="flex w-full items-center justify-between bg-white
                            dark:bg-gray-800 px-8 py-6 border-l-4
                            @if (($cash_out -> status)  == 'Success')
                            border-green-500
                            ...
                            <div class="flex">
                                ...
                                <div class="flex flex-col ml-6">
                                    <span class="text-lg font-bold">{{ $cash_out -> user-> username  }}</span>
                                    <div class="mt-4 flex">
                                        <div class="flex">
                                            <svg
                                        ...
                                        <div class="flex ml-6">
                                            ...
                                            <span
                                                class="ml-2 text-sm text-gray-600
                                                dark:text-gray-300 capitalize">
                                                {{ $cash_out -> user -> created_at ->diffForHumans()  }}
                                            </span>
                                        </div>
                                        ...
                                        <div class="flex ml-6">
                                            ...
                                            <span
                                                class="ml-2 text-sm text-gray-600
                                                dark:text-gray-300 capitalize">
                                                {{ $cash_out -> user-> designation }} [{{ $cash_out -> user -> email }} ]
                                            </span>
                                        </div>
                                    ...
                                    <div class="mt-4 flex">
                                        ...
                                        <form action="{{ url('update_cashout') }}" method="POST">
                                            @csrf
                                            @method('PUT')

                                            <input type="hidden" name="id" value="{{ $cash_out -> user -> id }}">
                                            <input type="hidden" name="status" value="Success">
                                            <button
                                                class="flex items-center ml-4
                                                focus:outline-none border rounded-full
                                                py-2 px-6 leading-none border-green-700
                            ...
                            <div class="flex flex-col -mt-10 mr-20">
                                <span
                                    ...
                                    dark:text-green-300">
                                    {{ $cash_out -> profile -> plan  }}
                                </span>
                                <span
                                    class="font-semibold text-black
                                    dark:text-green-300">
                                     # {{ $cash_out -> profile -> daily_earning  }} | 
                                     @php
                                        $user_earned = $cash_out -> profile -> daily_earning;
                                        $user_sub_wt_interest =  ($cash_out -> coupon_plan -> plan_amount)   * 1.5;
                                        $user_per_earned = ($user_earned / $user_sub_wt_interest) * 100;
                                        echo $perc_earned = round($user_per_earned) . '%';
                                    @endphp
                                </span>
                                <span
                                    class="font-semibold text-gray-400
                                    dark:text-green-300">
                                     # {{ $cash_out -> profile -> referral_earning  }}
                                </span>
                                <span
                                    class="text-sm text-green-700 dark:text-green-600
                                    mt-2 border-black border-t-2 border-b-2 ">
                                    # {{ $cash_out -> profile -> referral_earning + $cash_out -> profile -> daily_earning}}
                                    / {{ $cash_out -> amount}}
                                </span>
                        ...
                        <div
                            ...
                            justify-center dark:bg-gray-800 hover:bg-blue-200 dark-hover:bg-blue-500
                            @if ($cash_out -> status == 'Success')
                            bg-green-700
                            @elseif($cash_out -> status == 'Processing')
                            bg-gray-700
                            @elseif($cash_out -> status == 'Declined')
                            bg-black
                            @elseif($cash_out -> status == 'Hold')
                            bg-white
                            @endif
